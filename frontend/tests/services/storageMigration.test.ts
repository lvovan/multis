...
describe('migrateStorageKeys', () => {
  ...
  it('migrates old localStorage keys to new keys', () => {
    ...
    expect(localStorage.getItem('multis_players')).toBe('{"version":5,"players":[]}');
    expect(localStorage.getItem('multis_lang')).toBe('fr');
  });
  ...
  it('migrates old sessionStorage key to new key', () => {
    ...
    expect(sessionStorage.getItem('multis_session')).toBe(
  ...
  it('is idempotent — running twice produces the same result', () => {
    ...
    expect(localStorage.getItem('multis_players')).toBe('{"version":5,"players":[{"name":"Eve"}]}');
    expect(localStorage.getItem('multis_lang')).toBe('ja');
    expect(localStorage.getItem('turbotiply_players')).toBeNull();
    expect(localStorage.getItem('turbotiply_lang')).toBeNull();
  });
  ...
  it('does not overwrite existing new keys', () => {
    ...
    localStorage.setItem('multis_players', '{"version":5,"players":[{"name":"New"}]}');

    migrateStorageKeys();

    expect(localStorage.getItem('multis_players')).toBe('{"version":5,"players":[{"name":"New"}]}');
  });
  ...
  it('handles mixed state — new keys preserved, old keys cleaned up', () => {
    ...
    localStorage.setItem('multis_lang', 'pt');
    sessionStorage.setItem('turbotiply_session', '{"playerName":"Old"}');
    sessionStorage.setItem('multis_session', '{"playerName":"New"}');

    migrateStorageKeys();

    // New keys preserved
    expect(localStorage.getItem('multis_players')).toBe('{"version":5,"players":[{"name":"New"}]}');
    expect(localStorage.getItem('multis_lang')).toBe('pt');
    expect(sessionStorage.getItem('multis_session')).toBe('{"playerName":"New"}');
    // Old keys removed
    expect(localStorage.getItem('turbotiply_players')).toBeNull();
    expect(localStorage.getItem('turbotiply_lang')).toBeNull();
    expect(sessionStorage.getItem('turbotiply_session')).toBeNull();
